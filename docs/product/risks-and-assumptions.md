...
| R9 | **Data loss / no backups** on a free tier. | High | Documented periodic export (the user-facing export doubles as a backup); understand Supabase's free backup limits. | Open — scheduled for verification in M5 production hardening (see [the roadmap](../roadmap/roadmap.md)). |
